...
export default function Appointments() {
    const { profile, appointments, count } = useLoaderData() || {};
    const [appointmentList, setAppointmentList] = useState(appointments || []);
    const [totalCount, setTotalCount] = useState(count || 0);
    const [loading, setLoading] = useState(false);
    const [page, setPage] = useState(1);

    async function fetchAppointments(newPage = 1) {
        try {
            setLoading(true);
            const skip = (newPage - 1) * limit;

            const res = await fetch(
                `${apiPaths.appointments}/${profile._id}?skip=${skip}&limit=${limit}`,
    ...
    return (
        <div className="p-8">
            {/* Header */}
            <div className="mb-6">
                <h1 className="text-3xl font-heading font-bold text-primary-700">Your Appointments</h1>
                <p className="text-muted mt-1">Review and manage your upcoming sessions.</p>
            </div>

            {/* Loading State */}
            {loading && (
                <div className="text-center text-muted py-10">
                    <span className="animate-pulse text-primary-600 font-medium">Loading appointments...</span>
                </div>
            )}
            ...
            {!loading && (
                <>
                    {appointmentList?.length > 0 ? (
                        ...
                        <div className="text-center text-muted py-10">No appointments found.</div>
                    )}
...
export async function loader() {
    try {
        ...
        const res = await fetch(apiPaths.appointments + "/" + parsed._id, {
            method: "GET",
            headers: {
        ...
        return { profile: parsed, appointments: response?.appointments || [], count: response?.count || 0 };
    } catch (err) {
